# ===============================================================
# Origin: https://github.com/tqtg/hierarchical-attention-networks
# ===============================================================
...
def count_parameters(trained_vars, print_summary=True):
    total_parameters = 0
    if print_summary:
        print('=' * 100)
...
def batch_doc_normalize(docs):
    ...
    padded_docs = np.zeros(
        shape=[len(docs), max_sent_length, max_word_length], dtype=np.int32)  # PADDING 0
...
def load_glove(glove_file, emb_size, vocab):
    ...
    f.close()
    print('Total {} word vectors in {}'.format(
        len(embedding_weights), glove_file))

    embedding_matrix = np.random.uniform(-0.5,
                                         0.5, (len(vocab), emb_size)) / emb_size
